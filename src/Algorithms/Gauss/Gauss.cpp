...
void Gauss::FirstLevelParallelization(S21Matrix& matrix, double& tmp, int i,
    ...
    auto thread_func = [&matrix, &tmp, &threads, i](int thread_id) {
        int start_index = matrix.get_rows() - (double)(matrix.get_rows() - (i - 1)) / 2 * thread_id;
        int end_index = (i - 1) + (matrix.get_rows() - (i - 1)) / 2 * (2 - 1 - thread_id);
...
void Gauss::SecondLevelParallelization(S21Matrix& matrix, double& tmp, int i,
    ...
    int level = 2;
    auto thread_func = [&matrix, &tmp, &threads, i](int thread_id) {
        int start_index = (i + 1) + (matrix.get_rows() - (i + 1)) / 2 * (2 - 1 - thread_id);
        int end_index = matrix.get_rows() - (double)(matrix.get_rows() - (i + 1)) / 2 * thread_id;
...
void Gauss::ThirdLevelParallelization(S21Matrix& matrix, double& tmp, std::vector<std::thread*> threads) {
//    for (int i = 0; i < matrix.get_rows(); ++i) {
//        tmp = matrix(i, i);
//        FirstLevelParallelization(matrix, tmp, i, threads);
//        SecondLevelParallelization(matrix, tmp, i, threads);
//    }
    int level = 3;
    auto thread_func = [&matrix, &tmp, &threads](int thread_id) {
        int start_index = matrix.get_rows() * (thread_id / 2);
        int end_index = matrix.get_rows() - (double)matrix.get_rows() / 2 * thread_id;
